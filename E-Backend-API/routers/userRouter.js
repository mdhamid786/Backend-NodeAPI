...
router.route("/reset-password/:token").put(authMiddleware, resetPassword);  //done
router.route("/wishlist").get(authMiddleware, userWishlist);   //done
router.route("/update-order/:id").put(authMiddleware, updateOrderStatus);
router.route("/empty-cart").delete(authMiddleware, emptyCart);  // done
router.route("/cart").get( authMiddleware, getUserCart);  // done
router.route("/get-single-order/:id").post( authMiddleware, getOrderByUserId); //
router.route("/getallorders").get( authMiddleware, getAllOrders);
router.route("/get-orders").get(authMiddleware, getOrders);   // done
router.route("/cash-order").post(authMiddleware, createOrder); // done
router.route("/apply-coupon").post(authMiddleware, applyCoupon); //done
router.route("/cart").post( authMiddleware, addToCart); // done
